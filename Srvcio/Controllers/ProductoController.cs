...
{
    /// <summary>
    ///   API para la creación de productos
    ...
    {
        private readonly MdloDtos.IModelos.IProducto _dbContext;

        MdloDtos.Utilidades.RespuestaServicios respuesta = new MdloDtos.Utilidades.RespuestaServicios();
        public ProductoController(MdloDtos.IModelos.IProducto dbContext)
        {
        ...
        /// </summary>
        VldcionDtos.ValidacionProducto validacionProducto = new VldcionDtos.ValidacionProducto();
        ...
        {     
            ...
            try
            {
                ObjProducto = await this._dbContext.ListarProducto();
                if (ObjProducto != null)
                {
        ...
        public async Task<ActionResult<IEnumerable<MdloDtos.Producto>>> FiltrarProductoGeneral(string FiltroBusqueda)
        {
            ...
            try
            {
                ...
                validacion = await validacionProducto.ValidarFiltroBusquedas(Codigo);
                if (validacion == (int)MdloDtos.Utilidades.Constantes.TipoMensaje.TransaccionExitosa) //si fue exito)
                {
                    ObjProductoExiste = await this._dbContext.FiltrarProductoGeneral(FiltroBusqueda);
                    if (ObjProductoExiste != null)
                    {
                        respuesta.exito = MdloDtos.Utilidades.Constantes.RetornoExito;
                        respuesta.mensaje = MdloDtos.Utilidades.Mensajes.MensajeOperacion(validacion);
                        respuesta.datos = ObjProductoExiste;
                    }
                    else
                    {
        ...
        public async Task<ActionResult<IEnumerable<MdloDtos.Producto>>> FiltrarProductoEspecifico(string CodigoBusqueda)
        {
            ...
            try
            {
                ...
                if (validacion == (int)MdloDtos.Utilidades.Constantes.TipoMensaje.TransaccionExitosa) //si fue exito)
                {
                    ObjProductoExiste = await this._dbContext.FiltrarProductoEspecifico(CodigoBusqueda);
                    if (ObjProductoExiste != null)
                    {
                        //exito.
                        respuesta.exito = MdloDtos.Utilidades.Constantes.RetornoExito;
        ...
        public async Task<ActionResult<dynamic>> IngresarProducto([FromBody] MdloDtos.Producto objProducto)
        {
            ...
            try
            {
                validacion = await validacionProducto.ValidarIngreso(objProducto);
                if (validacion == (int)MdloDtos.Utilidades.Constantes.TipoMensaje.TransaccionExitosa) //si fue exito)
                {
                    var ObProducto = await this._dbContext.IngresarProducto(objProducto);
                    if (ObProducto != null)
                    {
                        respuesta.exito = MdloDtos.Utilidades.Constantes.RetornoExito;
                        respuesta.mensaje = MdloDtos.Utilidades.Mensajes.MensajeOperacion(validacion);
                        respuesta.datos = ObProducto;
                    }
                    else
                    {
                        //Error en la transaccion.
        ...
        public async Task<ActionResult<dynamic>> EditarProducto([FromBody] MdloDtos.Producto objProducto)
        {
            ...
            try
            {
                validacion = await validacionProducto.ValidarActualizacion(objProducto);

                if (validacion == (int)MdloDtos.Utilidades.Constantes.TipoMensaje.TransaccionExitosa) //si fue exito)
                {
                    var ObProducto = await this._dbContext.EditarProducto(objProducto);
                    if (ObProducto != null)
                    {
                        respuesta.exito = MdloDtos.Utilidades.Constantes.RetornoExito;
                        respuesta.mensaje = MdloDtos.Utilidades.Mensajes.MensajeOperacion(validacion);
        ...
        public async Task<ActionResult<dynamic>> EliminarProducto([FromBody] MdloDtos.Producto objProducto)
        {
            int operacion = Convert.ToInt32(MdloDtos.Utilidades.Constantes.TipoOperacion.Eliminacion);
            int validacion = 0; // para sacar el mensaje de la operacion del crud.
            try
            {
                validacion = await validacionProducto.ValidarEliminar(objProducto);
                if (validacion == (int)MdloDtos.Utilidades.Constantes.TipoMensaje.TransaccionExitosa) //si fue exito)
                {
                    var ObProducto = await _dbContext.EliminarProducto(objProducto.PrCdgo);
                    if (ObProducto != null)
                    {
                        respuesta.exito = MdloDtos.Utilidades.Constantes.RetornoExito;
                        respuesta.mensaje = MdloDtos.Utilidades.Mensajes.MensajeOperacion(validacion);
