...
namespace Srvcio.Controllers
{
    ...
    {
        private readonly MdloDtos.IModelos.IClasificacion _dbContex;

        MdloDtos.Utilidades.RespuestaServicios respuesta = new MdloDtos.Utilidades.RespuestaServicios();

        public ClasificacionController(MdloDtos.IModelos.IClasificacion dbContex)
        {
            _dbContex = dbContex;
        }
        ...
        public async Task<ActionResult<IEnumerable<ClasificacionDTO>>> ListarClasificacion([FromQuery] bool estado = true)
        {
            ...
            try
            {
                // Llamamos al servicio con el valor del parámetro `estado`
                ObjClasificacion = await this._dbContex.ListarClasificacion(estado);
            ...
            return Ok(ObjClasificacion);  // Devuelve 200 OK con los datos
        }
        #endregion
        ...
        public async Task<ActionResult<IEnumerable<ClasificacionDTO>>> FiltrarClasificacionGeneral(string FiltroBusqueda, bool estado)
        {
            ...
            try
            {
                ...
                {
                    ObjClasificacion = await this._dbContex.FiltrarClasificacionGeneral(FiltroBusqueda, estado);
                    if (ObjClasificacion != null)
                    {
                        //exito.
                        respuesta.exito = MdloDtos.Utilidades.Constantes.RetornoExito;
        ...
        public async Task<ActionResult<dynamic>> IngresarClasificacion([FromBody] ClasificacionDTO objClasificacion)
        {
            ...
            try
            {
                validacion = await validacionCLasificacion.ValidarIngreso(objClasificacion);
                if (validacion == (int)MdloDtos.Utilidades.Constantes.TipoMensaje.TransaccionExitosa) //si fue exito)
                {
                    var ObClasificacio = await this._dbContex.IngresarClasificacion(objClasificacion);
            ...
            try
            {
                ...
                if (validacion == (int)MdloDtos.Utilidades.Constantes.TipoMensaje.TransaccionExitosa) //si fue exito)
                {
                    ...
                    if (ObClasifiaccion != null)
                    {
                        respuesta.exito = MdloDtos.Utilidades.Constantes.RetornoExito;
                        respuesta.mensaje = MdloDtos.Utilidades.Mensajes.MensajeOperacion(validacion);
                        respuesta.datos = ObClasifiaccion;
                    }
                    else
                    {
                        //Error en la transaccion.
