...
{
    /// <summary>
    ///   API para las Responsable
    ///   Jesus Al berto Calzada
    /// </summary>
    [ApiController]
    public class ResponsableController : Controller
    {
        private readonly MdloDtos.IModelos.IResponsable _dbContex;

        MdloDtos.Utilidades.RespuestaServicios respuesta = new MdloDtos.Utilidades.RespuestaServicios();

        public ResponsableController(MdloDtos.IModelos.IResponsable dbContex)
        {
            _dbContex = dbContex;
        }
        ...
        /// </summary>
        VldcionDtos.ValidacionResponsable validacionResponsable = new VldcionDtos.ValidacionResponsable();
        ...
        public async Task<ActionResult<IEnumerable<MdloDtos.Responsable>>> ListarResponsable([FromQuery] bool estado = true)
        {
            ...
            try
            {
                ObjResponsable = await this._dbContex.ListarResponsable(estado);
                if (ObjResponsable != null)
                {
                    ...
                    respuesta.datos = ObjResponsable;
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<IEnumerable<MdloDtos.Responsable>>> FiltrarResponsableGeneral(string FiltroBusqueda, bool estado)
        {
            ...
            try
            {
                ...
                validacion = await validacionResponsable.ValidarFiltroBusquedas(Codigo);
                if (validacion == (int)MdloDtos.Utilidades.Constantes.TipoMensaje.TransaccionExitosa) //si fue exito)
                {
                    ObjResponsable = await this._dbContex.FiltrarResponsableGeneral(FiltroBusqueda, estado);
                    if (ObjResponsable != null)
                    {
                        //exito.
                        respuesta.exito = MdloDtos.Utilidades.Constantes.RetornoExito;
        ...
        public async Task<ActionResult<IEnumerable<MdloDtos.Responsable>>> FiltrarResponsableEspecifico(string CodigoBusqueda, bool estado)
        {
            ...
            try
            {
                ...
                if (validacion == (int)MdloDtos.Utilidades.Constantes.TipoMensaje.TransaccionExitosa) //si fue exito)
                {
                    ...
                    ObjResponsable = await this._dbContex.FiltrarResponsableEspecifico(Codigo, estado);
                    if (ObjResponsable != null)
                    {
                        //exito.
                        respuesta.exito = MdloDtos.Utilidades.Constantes.RetornoExito;
        ...
        public async Task<ActionResult<dynamic>> IngresarResponsable([FromBody] MdloDtos.Responsable objResponsable)
        {
            ...
            try
            {
                validacion = await validacionResponsable.ValidarIngreso(objResponsable);
                if (validacion == (int)MdloDtos.Utilidades.Constantes.TipoMensaje.TransaccionExitosa) //si fue exito)
                {
                    var ObResponsable = await this._dbContex.IngresarResponsable(objResponsable);
                    if (ObResponsable != null)
                    {
                        respuesta.exito = MdloDtos.Utilidades.Constantes.RetornoExito;
                        respuesta.mensaje =MdloDtos.Utilidades.Mensajes.MensajeOperacion(validacion);
                        respuesta.datos = ObResponsable;
                    }
                    else
                    {
                        //Error en la transaccion.
        ...
        public async Task<ActionResult<dynamic>> EditarResponsable([FromBody] MdloDtos.Responsable ObjResponsable)
        {
            ...
            try
            {
                validacion = await validacionResponsable.ValidarActualizacion(ObjResponsable);
                if (validacion == (int)MdloDtos.Utilidades.Constantes.TipoMensaje.TransaccionExitosa) //si fue exito)
                {
                    var ObResponsable = await this._dbContex.EditarResponsable(ObjResponsable);
                    if (ObResponsable != null)
                    {
                        respuesta.exito = MdloDtos.Utilidades.Constantes.RetornoExito;
                        respuesta.mensaje = MdloDtos.Utilidades.Mensajes.MensajeOperacion(validacion);
        ...
        public async Task<ActionResult<dynamic>> EliminarResponsable([FromBody] MdloDtos.Responsable ObjResponsable)
        {
            ...
            try
            {
                validacion = await validacionResponsable.ValidarEliminar(ObjResponsable);
                if (validacion == (int)MdloDtos.Utilidades.Constantes.TipoMensaje.TransaccionExitosa) //si fue exito)
                {
                    var ObResponsable = await _dbContex.InactivarResponsable(ObjResponsable);
                    if (ObResponsable != null)
                    {
                        respuesta.exito = MdloDtos.Utilidades.Constantes.RetornoExito;
                        respuesta.mensaje = MdloDtos.Utilidades.Mensajes.MensajeOperacion(validacion);
